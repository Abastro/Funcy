...
None::Iterator<F>             Iterator<F> = (Integer size, Integer index,
                                            Pointer<F> pointer,
                                            value -> OffsetGet<F>(ite(pointer, index)))
...
Setter<F> = (Iterator<F> ite, Integer index, F value |
    <Iterator<F>> (ite, pointer -> OffsetSet<F>(ite(pointer), index, value))      
)

(FromInteger<F> fi, None::Iterable<F, Iterator<F>>) Array<F> = (Iterator<F> head,
                                                        hasNext -> HasNext<F, Iterator<F>>,
                                                        next -> Next<F, Iterator<F>> next,
                                                        indexer -> FromInteger<F>,
                                                        setter -> Setter<F>,
                                                        indexer expose fi) // Complex compound declaration with inheritance

Set<F, Array<F> L> = (L list, Integer index, F value |
    <Array<F>>(list, head -> Setter<F>(list(head), index, value)) // Syntax sugar for compound creation
)

AsArray<F> = (F... lists | <Array<F>> (
    <Array::Iterator<F>> (Size<F>(lists), -1, Pointerize<F>(lists))
